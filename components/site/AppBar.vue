<template>
  <div>
    <!-- Mobile Toggle -->
    <div class="mobile-toggle" @click="openSidebar" ref="open">
      <svg class="icon-navicon-round">
        <use xlink:href="/img/icons.svg#icon-navicon-round"></use>
      </svg>
    </div>
    <div class="mobile-toggle hide" @click="closeSidebar" ref="close">
      <svg class="icon-close-round">
        <use xlink:href="/img/icons.svg#icon-close-round"></use>
      </svg>
    </div>

    <!-- Navigation -->
    <nav class="navbar">
      <div class="navbar__container">
        <!-- Left Navigation Items -->
        <div class="navbar__left">
          <!-- Brand Logo -->
          <div class="navbar__item">
            <nuxt-link to="/" class="navbar__item__link brand-logo">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="100px"
                height="38px"
                viewBox="0 0 100 38"
                version="1.1"
              >
                <g id="Symbols" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                  <g id="header" transform="translate(-80.000000, -31.000000)">
                    <g id="logo">
                      <g transform="translate(80.000000, 31.000000)">
                        <path
                          d="M91.1701497 24.9325137C89.1187284 24.9325137 87.4499112 23.2636 87.4499112 21.2122751 87.4499112 19.1608538 89.1187284 17.4920365 91.1701497 17.4920365 93.2213782 17.4920365 94.8902919 19.1608538 94.8902919 21.2122751 94.8902919 23.2636 93.2213782 24.9325137 91.1701497 24.9325137M91.1701497 12.3904883C89.1866269 12.3904883 87.4354442 13.0563563 85.9602919 14.1654934 84.9538706 12.9602954 83.3299975 12.168468 81.7868503 12.1301787L81.7868503 12.1751228 81.7868503 32.3295898C81.7868503 35.4293868 83.4706168 37.9418234 87.1575812 37.9430772L87.0931548 37.9318893 87.028632 37.9205086 86.9949721 37.9205086 86.9949721 37.6228741 87.0785914 29.0295695C88.2985457 29.6678538 89.6915254 30.0340619 91.1610838 30.0340619 96.0253731 30.0340619 99.9873071 26.0765645 99.9873071 21.2122751 99.9873071 16.3478893 96.0344391 12.3904883 91.1701497 12.3904883"
                          id="path-p"
                          fill="#06ED06"
                        ></path>
                        <path
                          d="M58.9754888 25.0113685C57.9499711 25.0113685 56.9947629 24.6028203 56.2857832 23.8611452 55.6212655 23.1660538 55.2553467 22.2532822 55.2553467 21.2910335 55.2553467 19.2398051 56.9242604 17.5709878 58.9754888 17.5709878 61.0269102 17.5709878 62.6957274 19.2398051 62.6957274 21.2910335 62.6957274 23.3425513 61.0269102 25.0113685 58.9754888 25.0113685M71.4583924 12.470597C69.4448746 12.470597 67.5892401 13.1514142 66.1029 14.2912213 67.2822503 15.6042467 68.1345497 17.2199218 68.5334533 19.0010995 68.5253518 19.0115157 68.518697 19.0280081 68.5105954 19.0384244 69.1913162 18.1531401 70.2603315 17.5909523 71.4608036 17.5909523 73.5122249 17.5909523 75.1811386 19.2798305 75.1811386 21.3311553 75.1811386 23.3825766 73.5123213 25.0915157 71.4609964 25.0915157 70.4353822 25.0915157 69.4802706 24.7630183 68.7711944 24.0214396 68.1948289 23.4184548 67.8440523 22.8407391 67.7609152 22.010333 67.780301 21.772399 67.7973721 21.5338863 67.7973721 21.2910335 67.7973721 16.4268406 63.8399711 12.4693431 58.9754888 12.4693431 54.1111995 12.4693431 50.1537985 16.4268406 50.1537985 21.2910335 50.1537985 23.5726731 51.0218188 25.7374193 52.5980472 27.3863685 54.2789203 29.1445919 56.5437782 30.1129168 58.9754888 30.1129168 61.3659203 30.1129168 63.5341386 29.1537543 65.1245447 27.6044345 66.7993416 29.3200284 69.0474178 30.1133025 71.4505802 30.1133025 76.3150624 30.1133025 80.2776716 26.1560944 80.2776716 21.2917086 80.2776716 16.4274193 76.3227782 12.470597 71.4583924 12.470597"
                          id="path-oo"
                          fill="#06ED06"
                        ></path>
                        <path
                          d="M48.4287046 19.3816878L52.6725523 7.72186041C52.7377503 7.57940863 52.8015015 7.43589594 52.8560904 7.28611421 53.9292528 4.33764213 52.4607553 1.09558629 49.5596386 0.0158654822L47.5904863 5.42613959 47.5302071 5.40424619 41.1732122 22.8699721 41.194334 22.8775914C40.1557959 25.8136218 41.6230396 29.0260685 44.5096893 30.0983629L46.4425777 24.7878147 46.4588772 24.7937944 48.4284152 19.3824594C48.4346843 19.3847741 48.4410497 19.3860279 48.4473188 19.3882462 48.4493442 19.3890178 48.4390244 19.3853528 48.4287046 19.3816878"
                          id="path-l"
                          fill="#06ED06"
                        ></path>
                        <path
                          d="M36.5690299 14.1400604C35.5912533 13.164502 34.335035 12.5457964 32.7918878 12.5025883L32.7918878 29.9370655C35.9746289 29.856436 38.1411112 27.5372787 38.1411112 24.4838726L38.1892381 20.0302533C38.1892381 17.921832 39.6396036 16.444365 41.761431 16.444365L41.761431 12.1483396C39.6396036 12.1483396 37.9457102 12.9061213 36.5690299 14.1400604"
                          id="path-r"
                          fill="#FFFFFF"
                        ></path>
                        <path
                          d="M22.0014117 25.0113492C19.9500868 25.0113492 18.2811731 23.3424355 18.2811731 21.2911107 18.2811731 19.2397858 19.9500868 17.5708721 22.0014117 17.5708721 24.0527365 17.5708721 25.7216503 19.2397858 25.7216503 21.2911107 25.7216503 23.3424355 24.0527365 25.0113492 22.0014117 25.0113492M22.0014117 12.4693239C17.1370259 12.4693239 13.1796249 16.4267249 13.1796249 21.2911107 13.1796249 26.1554 17.1370259 30.1129939 22.0014117 30.1129939 26.8657975 30.1129939 30.8232949 26.1554 30.8232949 21.2911107 30.8232949 16.4267249 26.8657975 12.4693239 22.0014117 12.4693239"
                          id="path-o"
                          fill="#FFFFFF"
                        ></path>
                        <path
                          d="M12.0886777 6.38239898L13.6953832 6.38239898 13.6953832 0.981383756 12.0886777 0.981383756C7.20288071 0.981383756 3.18269289 4.95874924 3.18269289 9.84444975L3.18269289 12.0727543 0.0963984772 12.0727543 0.0963984772 17.3773228 3.18269289 17.3773228 3.18269289 24.4217898C3.18269289 27.5235157 5.88320051 30.1129168 8.58370812 30.1129168L8.58370812 30.0433787 8.65382487 30.1129168C8.64649492 30.0243787 8.58370812 29.9349726 8.58370812 29.8444091L8.58370812 18.8805411 8.58370812 17.3773228 11.7664492 17.3773228 11.7664492 12.0727543 8.58370812 12.0727543 8.58370812 9.70662741C8.58370812 7.92708934 10.309236 6.38239898 12.0886777 6.38239898"
                          id="path-f"
                          fill="#FFFFFF"
                        ></path>
                      </g>
                    </g>
                  </g>
                </g>
              </svg>
            </nuxt-link>
          </div>
          <!-- Nav ITEM -->
          <div class="navbar__item navbar__item--hide--xs">
            <nuxt-link to="/meetups" class="navbar__item__link">
              <span class="link-caption">Find a</span>
              <span class="link-text">Meetup</span>
            </nuxt-link>
          </div>
          <!-- Nav ITEM -->
          <div class="navbar__item navbar__item--hide--xs">
            <nuxt-link to="/about" class="navbar__item__link">
              <span class="link-caption">About the</span>
              <span class="link-text">Community</span>
            </nuxt-link>
          </div>
          <!-- Nav ITEM -->
          <div class="navbar__item navbar__item--hide">
            <nuxt-link to="/team" class="navbar__item__link">
              <span class="link-caption">The</span>
              <span class="link-text">Team</span>
            </nuxt-link>
          </div>
          <!-- Nav ITEM -->
          <div class="navbar__item navbar__item--hide">
            <nuxt-link to="/members" class="navbar__item__link">
              <span class="link-caption">Become a</span>
              <span class="link-text">Member</span>
            </nuxt-link>
          </div>
          <!-- Nav ITEM -->
          <div v-if="currentUser" class="navbar__item navbar__item--hide">
            <nuxt-link to="/profile" class="navbar__item__link">
              <span class="link-caption">My</span>
              <span class="link-text">Profile</span>
            </nuxt-link>
          </div>
          <div v-else class="navbar__item navbar__item--hide">
            <a to="#" class="navbar__item__link" @click="login">
              <span class="link-caption">Member</span>
              <span class="link-text">Login</span>
            </a>
          </div>
          <div v-if="currentUser" class="sidebar-nav__item">
            <a to="#" class="navbar__item__link" @click="logout">
              <span class="link-text">Logout</span>
            </a>
          </div>
        </div>
        <div class="navbar__right navbar__right--hide">
          <!-- Nav ITEM -->
          <div class="navbar__item">
            <div class="navbar__item__link">
              <span class="link-caption">Stay in the loop</span>
              <span class="link-text">
                <a href class="social-link social-link--facebook">
                  <svg class="icon-social-facebook icon-sm">
                    <use xlink:href="/img/icons.svg#icon-social-facebook"></use>
                  </svg>
                </a>
                <a
                  href="https://twitter.com/forloopnigeria"
                  class="social-link social-link--twitter"
                >
                  <svg class="icon-social-twitter icon-sm">
                    <use xlink:href="/img/icons.svg#icon-social-twitter"></use>
                  </svg>
                </a>
                <a
                  href="https://github.com/forLoopOfficial"
                  class="social-link social-link--github"
                >
                  <svg class="icon-social-github icon-sm">
                    <use xlink:href="/img/icons.svg#icon-social-github"></use>
                  </svg>
                </a>
              </span>
            </div>
          </div>
        </div>
      </div>
    </nav>
    <nav class="sidebar-nav" ref="sidebar">
      <div class="sidebar-nav__container">
        <div @click="closeSidebar" class="navbar__item">
          <nuxt-link to="/" class="navbar__item__link brand-logo">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="100px"
              height="38px"
              viewBox="0 0 100 38"
              version="1.1"
            >
              <g id="Symbols" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                <g id="header" transform="translate(-80.000000, -31.000000)">
                  <g id="logo">
                    <g transform="translate(80.000000, 31.000000)">
                      <path
                        d="M91.1701497 24.9325137C89.1187284 24.9325137 87.4499112 23.2636 87.4499112 21.2122751 87.4499112 19.1608538 89.1187284 17.4920365 91.1701497 17.4920365 93.2213782 17.4920365 94.8902919 19.1608538 94.8902919 21.2122751 94.8902919 23.2636 93.2213782 24.9325137 91.1701497 24.9325137M91.1701497 12.3904883C89.1866269 12.3904883 87.4354442 13.0563563 85.9602919 14.1654934 84.9538706 12.9602954 83.3299975 12.168468 81.7868503 12.1301787L81.7868503 12.1751228 81.7868503 32.3295898C81.7868503 35.4293868 83.4706168 37.9418234 87.1575812 37.9430772L87.0931548 37.9318893 87.028632 37.9205086 86.9949721 37.9205086 86.9949721 37.6228741 87.0785914 29.0295695C88.2985457 29.6678538 89.6915254 30.0340619 91.1610838 30.0340619 96.0253731 30.0340619 99.9873071 26.0765645 99.9873071 21.2122751 99.9873071 16.3478893 96.0344391 12.3904883 91.1701497 12.3904883"
                        id="path-p"
                        fill="#06ED06"
                      ></path>
                      <path
                        d="M58.9754888 25.0113685C57.9499711 25.0113685 56.9947629 24.6028203 56.2857832 23.8611452 55.6212655 23.1660538 55.2553467 22.2532822 55.2553467 21.2910335 55.2553467 19.2398051 56.9242604 17.5709878 58.9754888 17.5709878 61.0269102 17.5709878 62.6957274 19.2398051 62.6957274 21.2910335 62.6957274 23.3425513 61.0269102 25.0113685 58.9754888 25.0113685M71.4583924 12.470597C69.4448746 12.470597 67.5892401 13.1514142 66.1029 14.2912213 67.2822503 15.6042467 68.1345497 17.2199218 68.5334533 19.0010995 68.5253518 19.0115157 68.518697 19.0280081 68.5105954 19.0384244 69.1913162 18.1531401 70.2603315 17.5909523 71.4608036 17.5909523 73.5122249 17.5909523 75.1811386 19.2798305 75.1811386 21.3311553 75.1811386 23.3825766 73.5123213 25.0915157 71.4609964 25.0915157 70.4353822 25.0915157 69.4802706 24.7630183 68.7711944 24.0214396 68.1948289 23.4184548 67.8440523 22.8407391 67.7609152 22.010333 67.780301 21.772399 67.7973721 21.5338863 67.7973721 21.2910335 67.7973721 16.4268406 63.8399711 12.4693431 58.9754888 12.4693431 54.1111995 12.4693431 50.1537985 16.4268406 50.1537985 21.2910335 50.1537985 23.5726731 51.0218188 25.7374193 52.5980472 27.3863685 54.2789203 29.1445919 56.5437782 30.1129168 58.9754888 30.1129168 61.3659203 30.1129168 63.5341386 29.1537543 65.1245447 27.6044345 66.7993416 29.3200284 69.0474178 30.1133025 71.4505802 30.1133025 76.3150624 30.1133025 80.2776716 26.1560944 80.2776716 21.2917086 80.2776716 16.4274193 76.3227782 12.470597 71.4583924 12.470597"
                        id="path-oo"
                        fill="#06ED06"
                      ></path>
                      <path
                        d="M48.4287046 19.3816878L52.6725523 7.72186041C52.7377503 7.57940863 52.8015015 7.43589594 52.8560904 7.28611421 53.9292528 4.33764213 52.4607553 1.09558629 49.5596386 0.0158654822L47.5904863 5.42613959 47.5302071 5.40424619 41.1732122 22.8699721 41.194334 22.8775914C40.1557959 25.8136218 41.6230396 29.0260685 44.5096893 30.0983629L46.4425777 24.7878147 46.4588772 24.7937944 48.4284152 19.3824594C48.4346843 19.3847741 48.4410497 19.3860279 48.4473188 19.3882462 48.4493442 19.3890178 48.4390244 19.3853528 48.4287046 19.3816878"
                        id="path-l"
                        fill="#06ED06"
                      ></path>
                      <path
                        d="M36.5690299 14.1400604C35.5912533 13.164502 34.335035 12.5457964 32.7918878 12.5025883L32.7918878 29.9370655C35.9746289 29.856436 38.1411112 27.5372787 38.1411112 24.4838726L38.1892381 20.0302533C38.1892381 17.921832 39.6396036 16.444365 41.761431 16.444365L41.761431 12.1483396C39.6396036 12.1483396 37.9457102 12.9061213 36.5690299 14.1400604"
                        id="path-r"
                        fill="#FFFFFF"
                      ></path>
                      <path
                        d="M22.0014117 25.0113492C19.9500868 25.0113492 18.2811731 23.3424355 18.2811731 21.2911107 18.2811731 19.2397858 19.9500868 17.5708721 22.0014117 17.5708721 24.0527365 17.5708721 25.7216503 19.2397858 25.7216503 21.2911107 25.7216503 23.3424355 24.0527365 25.0113492 22.0014117 25.0113492M22.0014117 12.4693239C17.1370259 12.4693239 13.1796249 16.4267249 13.1796249 21.2911107 13.1796249 26.1554 17.1370259 30.1129939 22.0014117 30.1129939 26.8657975 30.1129939 30.8232949 26.1554 30.8232949 21.2911107 30.8232949 16.4267249 26.8657975 12.4693239 22.0014117 12.4693239"
                        id="path-o"
                        fill="#FFFFFF"
                      ></path>
                      <path
                        d="M12.0886777 6.38239898L13.6953832 6.38239898 13.6953832 0.981383756 12.0886777 0.981383756C7.20288071 0.981383756 3.18269289 4.95874924 3.18269289 9.84444975L3.18269289 12.0727543 0.0963984772 12.0727543 0.0963984772 17.3773228 3.18269289 17.3773228 3.18269289 24.4217898C3.18269289 27.5235157 5.88320051 30.1129168 8.58370812 30.1129168L8.58370812 30.0433787 8.65382487 30.1129168C8.64649492 30.0243787 8.58370812 29.9349726 8.58370812 29.8444091L8.58370812 18.8805411 8.58370812 17.3773228 11.7664492 17.3773228 11.7664492 12.0727543 8.58370812 12.0727543 8.58370812 9.70662741C8.58370812 7.92708934 10.309236 6.38239898 12.0886777 6.38239898"
                        id="path-f"
                        fill="#FFFFFF"
                      ></path>
                    </g>
                  </g>
                </g>
              </g>
            </svg>
          </nuxt-link>
        </div>
        <!-- Nav ITEM -->
        <div @click="closeSidebar" class="sidebar-nav__item">
          <nuxt-link to="/meetups" class="navbar__item__link">
            <span class="link-caption">Find a</span>
            <span class="link-text">Meetup</span>
          </nuxt-link>
        </div>
        <!-- Nav ITEM -->
        <div @click="closeSidebar" class="sidebar-nav__item">
          <nuxt-link to="/about" class="navbar__item__link">
            <span class="link-caption">About the</span>
            <span class="link-text">Community</span>
          </nuxt-link>
        </div>
        <!-- Nav ITEM -->
        <div @click="closeSidebar" class="sidebar-nav__item">
          <nuxt-link to="/team" class="navbar__item__link">
            <span class="link-caption">The</span>
            <span class="link-text">Team</span>
          </nuxt-link>
        </div>
        <!-- Nav ITEM -->
        <div @click="closeSidebar" class="sidebar-nav__item">
          <nuxt-link to="/members" class="navbar__item__link">
            <span class="link-caption">Become a</span>
            <span class="link-text">Member</span>
          </nuxt-link>
        </div>
        <!-- Nav ITEM -->
        <div v-if="currentUser" @click="closeSidebar" class="sidebar-nav__item">
          <nuxt-link to="/profile" class="navbar__item__link">
            <span class="link-caption">My</span>
            <span class="link-text">Profile</span>
          </nuxt-link>
        </div>
        <div v-else class="sidebar-nav__item">
          <a to="#" class="navbar__item__link" @click="login">
            <span class="link-caption">Member</span>
            <span class="link-text">Login</span>
          </a>
        </div>
        <div v-if="currentUser" @click="logout" class="sidebar-nav__item">
          <nuxt-link to="#" class="navbar__item__link">
            <span class="link-text">Logout</span>
          </nuxt-link>
        </div>
        <!-- Nav ITEM -->
        <div class="sidebar-nav__item">
          <div class="navbar__item__link">
            <span class="link-caption">Stay in the loop</span>
            <span class="link-text">
              <a href class="social-link social-link--facebook">
                <svg class="icon-social-facebook icon-sm">
                  <use xlink:href="/img/icons.svg#icon-social-facebook"></use>
                </svg>
              </a>
              <a href="https://twitter.com/forloopnigeria" class="social-link social-link--twitter">
                <svg class="icon-social-twitter icon-sm">
                  <use xlink:href="/img/icons.svg#icon-social-twitter"></use>
                </svg>
              </a>
              <a href="https://github.com/forLoopOfficial" class="social-link social-link--github">
                <svg class="icon-social-github icon-sm">
                  <use xlink:href="/img/icons.svg#icon-social-github"></use>
                </svg>
              </a>
            </span>
          </div>
        </div>
      </div>
    </nav>
  </div>
</template>


<script>
import { mapActions, mapGetters } from 'vuex';

export default {
  name: 'AppBar',
  methods: {
    ...mapActions(['firebaseLogin', 'logout']),
    openSidebar() {
      this.$refs.open.classList.add('hide');
      this.$refs.sidebar.classList.add('active');
      this.$refs.close.classList.remove('hide');
    },
    closeSidebar() {
      this.$refs.open.classList.remove('hide');
      this.$refs.sidebar.classList.remove('active');
      this.$refs.close.classList.add('hide');
    },
    login() {
      return this.firebaseLogin()
        .then(() => {
          this.$toast.success('Succesful login');
        })
        .catch(error => {
          console.log(error);
          this.$toast.error(`${error.message}`);
        });
    }
  },
  computed: {
    ...mapGetters(['currentUser'])
  }
};
</script>

<style>

</style>
